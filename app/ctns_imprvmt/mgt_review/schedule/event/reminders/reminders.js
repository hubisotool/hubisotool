                        ...
                        b.execInDb(dbname,func,args).then(function(events){
                            events.forEach(function(event){
                                ...
                                event.reminders.forEach(function(reminder){
                                    log.debug({src:logSrc,diagId:"reminderDaemon::link::reminderDaemonHook",msg:"Checking Reminder Time" + JSON.stringify(reminder)});
                                    if(moment().isAfter(reminder.at)){
                                        log.debug({src:logSrc,diagId:"reminderDaemon::link::reminderDaemonHook",msg:"Current time is ahead of reminder time"});
                                        log.debug({src:logSrc,diagId:"reminderDaemon::link::reminderDaemonHook",msg:"Checking Reminder Status"});
                                        if(reminder.status==="active"){
                                            log.debug({src:logSrc,diagId:"reminderDaemon::link::reminderDaemonHook",msg:"Reminder status is active"});
                                            log.debug({src:logSrc,diagId:"reminderDaemon::link::reminderDaemonHook",msg:"Checking Reminder Type"});
                                            switch(reminder.type){
                                                case "popup":
                                                log.debug({src:logSrc,diagId:"reminderDaemon::link::reminderDaemonHook",msg:"Reminder type is popup"});
                                            ...
                                            log.debug({src:logSrc,diagId:"reminderDaemon::link::reminderDaemonHook",msg:"Reminder status is not active"});
                                        }
                                    }else{
                                        log.debug({src:logSrc,diagId:"reminderDaemon::link::reminderDaemonHook",msg:"Current time is not ahead of reminder time"});
                                    }
                                })
                            })
                        });
